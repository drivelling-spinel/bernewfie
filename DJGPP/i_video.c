...
{ 
 ...
 //M_DrawText2(1,0,CR_BLUE,true,mode_string); 
   ...
   else if (use_vsync && current_mode<256) wait_vsync(); // killough 2/7/98: use vsync() to prevent screen breaks.
...
void I_ReadScreen(byte *scr)
{
  ...
  else    memcpy(scr,vscreen,size);             // Others
}
...
vesa_mode_800x600=0x103;
  ...
  if (current_mode>255) 
  {
     ...
     if (use_vsync && !page_flip)      {use_vsync=0; }
	 if (vesa_version<2 && use_vsync)  {use_vsync=0; }
	 if (vesa_version<2 && page_flip)  {page_flip=0; }  // won't work, would just be wasting frames
	 if (vesa_version>=2)
	 {
		...
		else if (!in_page_flip && page_flip)                          // we need to check success.                         
		{
	       ...
	       if (status==1) {page_flip=use_vsync=0; } // function failed, cancel use
